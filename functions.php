...
function block_post($post_id){
	...
	if($post_id > 0) {
		...
		update_option('INTENTclick_disabled_pages', $disabled_pages);
	}
}
...
function add_post_to_immediately_list($post_id){
	$immediately_pages = get_option('INTENTclick_immediately_list');
	...
	if($post_id > 0) {
		...
		update_option('INTENTclick_immediately_list', $immediately_pages);
	}
}
 ...
 */
function INTENTclick_is_young_post() {
	global $post;
	$disable_new_days = get_option('INTENTclick_disable_new_days');
 ...
 */
function INTENTclick_is_page_enabled($id) {
	// get disabled pages from user's settings
	$disabled_pages = explode(',', get_option('INTENTclick_disabled_pages'));
 ...
 */
function INTENTclick_is_page_show_immediately($id) {
	// get immediately show pages from user's settings
	$immediately_pages = explode(',', get_option('INTENTclick_immediately_list'));
...
add_action('admin_head', 'INTENTclick_quickedit_get_id');
add_action('wp_ajax_INTENTclick_get_post_status', 'INTENTclick_get_post_status' );

/**
 * this function is updating the quick edit checkbox according to the post settings (ajax)
 */
function INTENTclick_quickedit_get_id() {?>
   ...
   jQuery(document).ready(function() {
		...
		var data = {
				action: "INTENTclick_get_post_status",
				post_id: qe_id, 
				qe_mode: "ajaxget"
			};
		jQuery.post(
 ...
 */
function INTENTclick_get_post_status(){
...
add_action('admin_head', 'INTENTclick_remove_from_block_list');
add_action('wp_ajax_INTENTclick_remove_from_block_list_call', 'INTENTclick_remove_from_block_list_call' );

/**
 * this function remove posts from the block list (ajax)
 */
function INTENTclick_remove_from_block_list() {?>
   <script type="text/javascript">
   jQuery(document).ready(function() {
		...
		var data = {
				action: "INTENTclick_remove_from_block_list_call",
				post_id: qe_id
			};
		jQuery.post(
				'<?php bloginfo('url');?>/wp-admin/admin-ajax.php', data ,
				function(response){ 
					if(response != 0){ 
						...
						if(response == 2){
							jQuery('.INTENTclick_block_pages').html("<b>No blocked posts.</b>");
 ...
 */
function INTENTclick_remove_from_block_list_call(){
